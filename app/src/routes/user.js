...
exports.register = async function (req, res, next) {
  try {
    ...
    password = CryptoJS.HmacSHA1(securityString, password).toString(
      CryptoJS.enc.Base64
    );
...
exports.login = async function (req, res, next) {
  try {
    ...
    if (fullName)
      userToLoginByFullName = await bdTotal.filter(
        (user) => user.dataValues.fullName == fullName
      );
    if (email)
      userToLoginByEmail = await bdTotal.filter(
        (user) => user.dataValues.email == email
      );

    userToLogin = userToLoginByFullName.length
      ? userToLoginByFullName
      : userToLoginByEmail;
    ...
    if (!userToLogin.length) {
      ...
      res
        .status(404)
        .send({
          info: "Sorry, theres no registered user with that email or username.",
        });
    } else {
      console.log(
        "userToLogin[0].securityString   :",
        userToLogin[0].securityString
      );
      console.log("password   :", password);
      ...
      if (
        ...
        if (!userToLogin[0].emailVerificated)
          res.status(401).send({ info: "Email verification required" });
        if (userToLogin[0].password != password)
          res.status(401).send({ info: "Password incorrect" });
      }
    }
  } catch (error) {
    next({ info: error });
...
exports.shippingDataRequirement = async function (req, res, next) {
  const { billing_address, shipping_address, phone } = req.body;
  if (billing_address && shipping_address && phone) {
    next();
  } else {
